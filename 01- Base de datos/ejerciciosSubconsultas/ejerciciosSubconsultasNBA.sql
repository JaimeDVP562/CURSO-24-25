...
--EJ08--------------------------------------->preguntar a fran<-------------- NO TENGO NI IDEA DE PORQUE NO SALEN IGUAL
SELECT *
FROM jugador
WHERE jugador.codigo IN (
	SELECT estadistica.jugador
...
--EJ12--------------------------------------->preguntar a fran<--------------NO SALEN LOS MISMOS RESULTADOS 
SELECT jugador.nombre
FROM jugador
WHERE jugador.codigo IN (
	SELECT estadistica.jugador
...
WHERE jugador.codigo IN (
	...
	FROM estadistica
	WHERE (
	SELECT SUM(estadistica.puntos_por_partido)
	FROM estadistica
	WHERE temporada= '07/08'
	) > (
...
WHERE jugador.codigo IN (
	...
	FROM estadistica
	WHERE 
) AND jugador.nombre_equipo = 'Raptors'
